...
class LandingActivity : BaseActivity<ActivityLandingBinding>() {
    ...
    private fun checkTestValue() {
        if (viewModel.getTestValue())
            binding.testValue.text = getString(BaseString.test_value_true)
        else
            binding.testValue.text = getString(BaseString.test_value_false)
    }

    private fun checkIfLoggedIn() =
        with(viewModel.isLoggedIn()) {
            if (first) {
                binding.loginStatus.text = getString(BaseString.login_status_success)
                goToWelcomeScreen(second!!)
            } else {
                binding.loginStatus.text = getString(BaseString.login_status_false)
            }
        }
